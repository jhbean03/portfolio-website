...
function WorkBox({title, department, location, date, skills, descriptionPoints, last}: WorkBoxProps) {
    return (
        <div className="grid grid-cols-2 max-lg:grid-cols-1 lg:gap-[4em] bg-slate-300 dark:bg-slate-700 p-[1em] rounded-4xl justify-items-center items-center">
            <div>
                <h2 className="font-head text-[24px] text-center font-bold p-0">{title}</h2>
                ...
                { skills == "" ? "" : <div className="text-center mt-[1em] bg-[#aec4e4] dark:bg-slate-500 p-[0.5em] rounded-4xl"><b>Relevant Skills:</b> {skills}</div>}
            </div>
            <div>
